...
public struct Flag: CommandComponent {
	...
	let isShortFlag: Bool

	public var stringRepresentation: String? {
		guard shouldAppend else {
			return nil
		}
		return isShortFlag ? "-\(name)" : "--\(name)"
	}
	...
	///   - isShortFlag: An optional bool that you can use to use a short flag like `-verbose` (instead of `--verbose`)
	public init(_ name: String, shouldAppend: Bool = true, isShortFlag: Bool = false) {
		self.name = name.components(separatedBy: "-").compactMap { $0.isEmpty ? nil : $0 }.joined(separator: "-")
		self.shouldAppend = shouldAppend
		self.isShortFlag = isShortFlag
	}
